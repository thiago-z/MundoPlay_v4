...
mysqli_query($strcon,$sql) or die("Erro no cadastro da notícia sobre cinema!");
...
$tituloRelacionado = "SELECT * FROM series WHERE idseries = $relacionado";	
$tituloresultado = mysqli_query($strcon,$tituloRelacionado) or die(mysqli_error()."<br>Erro ao executar a inserção da série relacionada");
	
	while($elemento = mysqli_fetch_array($tituloresultado))
 		{
   		$idSerieRelacionado = $elemento['idseries'];
		}
...
$diretorio = '../app_cinema_UC16_v4/img/noticias/series/';
$pasta = 'titulo'.$idSerieRelacionado;
